...
- **Follow-up fixes same day:** (1) rate-limit — the per-recipient send loop tripped Resend's ~2 req/sec limit and the SDK returns 429s in the response (doesn't throw), so failures were miscounted as sent (UI said 98, only ~37 delivered). Fixed with `sendAnnouncementBatch()` using Resend Batch API (`client.batch.send`, chunks of 50, `batchValidation: 'permissive'`), ban-filter once up front, real counts; `sendAnnouncement` now throws on `error`. Route added an exclude list + reports sent/failed/skippedBanned/excluded. (2) confirmation — `post()` now handles fetch failure/non-OK/non-JSON and shows instant "Sending…" + persistent ✓/⚠ banner. July giveaway fully sent: 37 first pass + 66 re-send (excluded the 37) = all 98 opted-in. RESEND_API_KEY is Vercel-only (not in local .env.local).

## 2026-07-01 — Giveaway winner fixes (unclickable past winners + admin saved-winner)

- **Past Winners names were links to `/sellers/{username}`** (seller storefront) — 404s for any winner who isn't a seller, and if their username is an email address (e.g. June winner's username is `junewaycao@gmail.com`, display_name `mozart007`). Made the winner name plain text (unclickable) in `src/app/giveaway/page.tsx`. Sponsors stay clickable.
