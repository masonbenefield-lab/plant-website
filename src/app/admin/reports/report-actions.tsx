...
export function ReportActions({ reportId, listingId, auctionId, reportedUserId, communityPostId, communityReplyId, targetName }: ReportActionsProps) {
  ...
  async function handleRemoveAndResolve() {
    ...
    if (listingId) {
      ...
      const { error } = await supabase.from("community_replies").delete().eq("id", communityReplyId);
      if (error) { toast.error(error.message); setLoading(false); return; }
      if (user) await auditLog(supabase, user.id, "delete_reply_via_report", "community_reply", communityReplyId, `report:${reportId}`);
  ...
  const removeLabel = listingId ? "Delete listing" : auctionId ? "Cancel auction" : communityReplyId ? "Delete reply" : communityPostId ? "Delete post" : "Archive user";
      ...
      <Dialog open={removeOpen} onOpenChange={setRemoveOpen}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            This will {listingId ? "permanently delete the listing" : auctionId ? "cancel the auction" : communityReplyId ? "permanently delete the reply" : communityPostId ? "permanently delete the post" : "archive the user and pause their content"} for <strong>{targetName}</strong>, then resolve the report.
          </p>
          <div className="space-y-1.5 mt-2">
