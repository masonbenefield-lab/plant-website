...
export default async function SearchPage({
  ...
  const [{ data: listings }, { data: auctions }] = q.trim()
    ? await Promise.all([
        supabase
          .from("listings")
          .select("id, plant_name, variety, price_cents, images, status, category")
          .eq("status", "active")
        ...
        supabase
          .from("auctions")
          .select("id, plant_name, variety, current_bid_cents, images, status, ends_at, category")
          .eq("status", "active")
          .or(`plant_name.ilike.%${q}%,variety.ilike.%${q}%`)
  ...
  const total = listingResults.length + auctionResults.length;
      ...
      {q.trim() && (
        <div className="flex gap-2 mb-8">
          {[
            ...
            { label: `Shop (${listingResults.length})`, value: "shop" },
            { label: `Auctions (${auctionResults.length})`, value: "auctions" },
          ].map(({ label, value }) => (
            <Link
              key={value}
        ...
        <div className="space-y-10">
          {showListings && listingResults.length > 0 && (
            <section>
              <h2 className="text-lg font-semibold mb-4">
                Shop listings
                <span className="text-sm font-normal text-muted-foreground ml-2">{listingResults.length} result{listingResults.length !== 1 ? "s" : ""}</span>
              </h2>
          ...
          {showAuctions && auctionResults.length > 0 && (
            <section>
              <h2 className="text-lg font-semibold mb-4">
                Live auctions
                <span className="text-sm font-normal text-muted-foreground ml-2">{auctionResults.length} result{auctionResults.length !== 1 ? "s" : ""}</span>
              </h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
